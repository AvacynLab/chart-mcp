...
/**
 * Base64 encoding of a 1x1 transparent PNG. Storing the bytes inline allows
 * the Playwright helper to upload an "image" fixture without depending on
 * binary assets that break the CI/PR pipeline.
 */
const SAMPLE_IMAGE_BASE64 =
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGMAAQAABQABDQottAAAAABJRU5ErkJggg==";

/** Lazily materialised buffer for the sample attachment image. */
const SAMPLE_IMAGE_BUFFER = Buffer.from(SAMPLE_IMAGE_BASE64, "base64");
...
export class ChatPage {
  ...
  async addImageAttachment() {
    // We intercept the file chooser and provide the inline PNG payload so the
    // UI behaves as if a real image was selected while keeping the repository
